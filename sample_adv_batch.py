...
def main(args):
    ...
    model_pth += "_Network.pth"
    print(model_pth)
    print(opt["model"]["which_networks"][0]["args"]["beta_schedule"]["test"])
    ...
    with torch.no_grad():
        for i_batch, (x, y) in enumerate(tqdm(testloader, disable=True)):
            ...
            for query in range(1, 1+args.max_queries):
                ...
                log_msg = F"Batch: {i_batch}\tQuery: {query}\tSuccessed: {success}\tRemain: {correct.sum()}\tSuccess_Rate: {success*100/total_img_num:.4f}\tTime: {(endtime - starttime).seconds}s"

                print(log_msg)

                queries = queries+[query]*succ.sum()

                if index_correct.size(0) == 0:
                    break
...
if __name__ == '__main__':
    ...
    parser.add_argument('-C', '--config', type=str,
                        default='config/adv_cifar10_e16.json', help='JSON file for configuration')
