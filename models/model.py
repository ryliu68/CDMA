...
class Palette(BaseModel):
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader,disable=True):
            ...
            if self.iter % self.opt['train']['log_iter'] == 0:
                ...
                for key, value in self.get_current_visuals().items():
                    self.writer.add_images(key, value[:16])
            if self.ema_scheduler is not None:
    ...
    def test(self):
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader,disable=True):
